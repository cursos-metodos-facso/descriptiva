---
title: "Lectura 4. Análisis descriptivo de datos en R"
date: "2025-06-12"
lang: es
output:
...
[**1.  Establecer un flujo de trabajo ordenado en un script (.R).**]{.text-black}

:::: {.card .bg-light .text-black}
**2.  Aplicar análisis estadísticos descriptivos a <br> variables según su nivel de medición:**
...
- **Tablas de contingencia o cruzadas**
:::
...
::: {.callout-note}
Recordar: la función `load()` proviene de *r base* y nos permite cargar bases de datos en formato **.Rdata**.
...
*Otra forma de calcularlo es utilizando el lenguaje orientado a objetos*:
...
```{r eval=TRUE, include=TRUE, collapse=FALSE}
var(proc_elsoc$ingreso)
```

- **Desviación estándar**

```{r eval=TRUE, include=TRUE, collapse=FALSE}
sd(proc_elsoc$ingreso)
```
...
```{r eval=TRUE, include=TRUE, collapse=FALSE}
quantile(proc_elsoc$ingreso, probs = .25)
```

- **Segundo cuartil (Q2) o Mediana**

```{r eval=TRUE, include=TRUE, collapse=FALSE}
quantile(proc_elsoc$ingreso, probs = .50)
```
...
```{r eval=TRUE, include=TRUE, collapse=FALSE}
quantile(proc_elsoc$ingreso, probs = .75)
```
...
```{r eval=TRUE, include=TRUE, collapse=FALSE}
quantile(proc_elsoc$ingreso, probs = seq(0, 1, 0.01))
```


#### 4.2.2 Para un conjunto de estadísticos
...
Hoy aprendimos a *analizar datos en R*. En detalle, vimos:

[**1.  Cómo establecer un flujo de trabajo de procesamiento y análisis de datos en R.**]{.text-black}

[**2.  Realizar análisis descriptivos en R según el nivel de medición de las variables**]{.text-black}
